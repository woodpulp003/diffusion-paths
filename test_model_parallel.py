...
def create_individual_plots(original_data, checkpoint_samples, save_dir="test_results/individual_plots"):
    ...
    for epoch, samples in checkpoint_samples.items():
        ...
        plt.title(f'Diffusion Model Output - Epoch {epoch}')
        plt.xlabel('X')
        plt.ylabel('Y')
        plt.axis('equal')
...
def create_metrics_plots(metrics_data, save_dir="test_results"):
    ...
    plt.title(f'Best Model Distribution (Epoch {best_epoch})')
    plt.xlabel('X')
    plt.ylabel('Y')
    plt.axis('equal')
    plt.grid(True, alpha=0.3)
...
def main():
    ...
    for result in valid_results:
        ...
        np.save(f"test_results/samples_epoch_{epoch:04d}.npy", result['small_samples'])
        
        # Save large samples for robust evaluation
        np.save(f"test_results/samples_epoch_{epoch:04d}_large.npy", result['large_samples'])
    ...
    for result in valid_results:
        epoch = result['epoch']
        samples = np.load(f"test_results/samples_epoch_{epoch:04d}.npy")
    ...
    print(f"Parallel test completed successfully!")
